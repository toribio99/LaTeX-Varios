...
\begin{minipage}{0.45\textwidth}
	\begin{tikzpicture}
		...
		\node[red] at (1.5,4) {$y=f(x)$};

		% Punto P(2, f(2))
		\node[circle, fill=blue, inner sep=2pt, label={right:$P(2,f(2))$}] at (2,3.2) {};

		% Recta tangente: y - f(2) = f'(2)(x - 2)
		% f'(x) = 0.6x + 0.5, f'(2) = 1.7
		\addplot[green!60!black, thick, dashed, domain=0.5:3.5] {1.7*(x-2) + 3.2};
		\node[green!60!black] at (1.5,1) {\small Tangente};
...
\begin{center}
	\begin{tikzpicture}
		\begin{axis}[
			...
			legend style={
				font=\scriptsize, % texto más pequeño
				cells={align=left}, % opcional: alinea las entradas
				line width=0.8pt, % grosor de las líneas dentro de la leyenda
				mark size=1.5pt,  % tamaño de las marcas (si existieran)
			}
		]
		...
		\node[blue, below right] at (1,1) {$(1,1)$};
		...
		\node[green!60!black, below left] at (-1.5,2.25) {$(-1.5,2.25)$};
...
\begin{center}
	\begin{tikzpicture}
		\begin{axis}[
			width=6cm, height=8cm,        % << alto y angosto como en la imagen
			scale only axis=true,           % << respeta exactamente width/height
			axis lines=middle,
			xlabel={$x$}, ylabel={$y$},
			xmin=-2, xmax=2,                % << dominio visible
			ymin=-12, ymax=4,              % << rango visible
			xtick={-2,-1.5,...,1.5,2},
			ytick={-12,-10,...,2,4},
			grid=both,
			grid style={line width=.1pt, draw=gray!30},
			axis line style={-{Latex},thick},
			tick label style={font=\tiny},
			samples=300,                    % << más suave
			legend pos=south east,
			% Evita “picos” al salir del rango:
			restrict y to domain=-24:24,
			...
			% f(x)
			\addplot[red, very thick, domain=-2:2] {7*x^5 - 3*x^3 + 2*x - 9};
			\addlegendentry{$f(x)$}
			
			% f'(x)
			\addplot[blue, thick, dashed, domain=-2:2] {35*x^4 - 9*x^2 + 02};
			\addlegendentry{$f'(x)$}
...
\begin{center}
	\begin{tikzpicture}
		...
		\node[red] at (1.1,1) {$f(x) = x^3 - 6x^2 + 9x + 1$};
