...
\begin{tikzpicture}[scale=1.2]
    ...
    \node[green!60!black, above left=4mm] at (axis cs:0,0.5) {Lado recto: $4p = 4$};

    % Parámetro p
    \draw[{Latex}-{Latex}, thick, orange] (axis cs:-0.3,0) -- (axis cs:-0.3,1);
    \node[orange] at (axis cs:-0.9,0.65) {$p=1$};
...
\begin{tikzpicture}[scale=0.8]
    ...
    \begin{axis}[
        ...
        axis equal image,
	  	legend style={
		at={(0.65,1.025)}, % x,y relativo dentro del área 	del eje
		anchor=south east, % qué parte de la caja toca 	ese punto
		font=\scriptsize,
		fill=white
		}
    ]
...
\begin{tikzpicture}
    ...
    \node[blue, above left=2mm] at (axis cs:6.5,-0.3) {Directriz: $y = 0$};

    % Eje de simetría
    \addplot[dotted, thick] coordinates {(3,-1.5) (3,5.5)};
    \node[left=7mm, rotate=90] at (axis cs:3.5,5) {Eje: $x = 3$};

    % Lado recto
    \addplot[green!60!black, very thick] coordinates {(1,2) (5,2)};

    % Ecuación
    \node[maincolor] at (axis cs:5.6,3.5) {$(x-3)^2 = 4(y-1)$};
...
\node[above left, rotate=-45] at (axis cs:-0.6,-0.2) {$V(0,0)$};

% Foco
\addplot[only marks, mark=*, mark size=3pt, blue] coordinates {(2.5,0)};
\node[above right, rotate=45] at (axis cs:2.5,0) {$F(2.5,0)$};
...
\node[below] at (axis cs:1.6,4) {$(1.6, 4)$};
...
\node[below] at (axis cs:50,10) {$V(50,10)$};
...
% Diámetro
\draw[{Latex}-{Latex}, thick, orange] (axis cs:-6,9.33) -- (axis cs:14,9.33)
...
\node[below right] at (axis cs:0,15) {$F(0,15)$};
...
\pgfplotsinvokeforeach{-25,-15,-5,5,15,25}{
    \draw[-{Latex}, yellow!80!orange, thick] (axis cs:#1,28) -- (axis cs:#1,{#1*#1/60});
    \draw[-{Latex}, yellow!80!orange, thick] (axis cs:#1,{#1*#1/60}) -- (axis cs:0,15);
}

% Diámetro del espejo
\draw[{Latex}-{Latex}, thick, orange] (axis cs:-30,15) -- (axis cs:30,15)
    node[midway,above right] {60 cm};
...
% Alcance
\draw[{Latex}-{Latex}, thick, orange] (axis cs:0,-2) -- (axis cs:80,-2)
...
\pgfplotsinvokeforeach{-3.5,-2.5,-1.5,-0.5,0.5,1.5,2.5,3.5}{
    \draw[-{Latex}, yellow!80!orange, thick]
        (axis cs:#1,3.5) -- (axis cs:#1,{#1*#1/8});
    \draw[-{Latex}, red!80!orange, thick]
...
% Dimensiones
\draw[{Latex}-{Latex}, thick, orange] (axis cs:-4,2.5) -- (axis cs:4,2.5)
    node[midway,above right] {8 m};
\draw[{Latex}-{Latex}, thick, orange] (axis cs:4.5,0) -- (axis cs:4.5,2)
    node[midway,left] {2 m};

% Ángulo de apertura
\draw[dashed, gray] (axis cs:0,0) -- (axis cs:4,2);
\draw[blue,-{Latex},thick] (axis cs:0,0.5) arc (90:45:0.5);
...
% Vértice
\addplot[only marks, mark=*, mark size=3pt, red] coordinates {(0,0)};
\node[above left] at (axis cs:0,0) {Vértice};

% Foco (LNB)
\addplot[only marks, mark=*, mark size=4pt, blue] coordinates {(0.9,0)};
\node[below right] at (axis cs:0.9,0) {LNB};
...
% Diámetro
\draw[{Latex}-{Latex}, thick, orange] (axis cs:0.4,-1.3) -- (axis cs:0.4,1.3)
...
\node[above left] at (axis cs:0,25) {Altura máxima};

% Río
\addplot[thick, cyan, domain=-45:45] coordinates {(-45,0) (45,0)};
\node[below] at (axis cs:0,-2) {Nivel del río};

% Dimensiones
\draw[{Latex}-{Latex}, thick, orange] (axis cs:-40,-1) -- (axis cs:40,-1)
    node[midway,above left=2mm] {80 m};
...
\draw[-{Latex}, very thick, red] (axis cs:0,0) -- (axis cs:50,86.6)
...
\begin{tikzpicture}[scale=0.7]
    ...
    % Dimensiones
    \draw[{Latex}-{Latex}, thick, gray] (axis cs: 0,-4.5) -- (axis cs: 2,-4.5);
    \node[gray] at (axis cs: 1,-4.5) [below] {2 m};
    \draw[{Latex}-{Latex}, thick, gray] (axis cs: 2.5,-4) -- (axis cs: 2.5,4);
