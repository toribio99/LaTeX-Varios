...
\begin{tikzpicture}[scale=1.2]
    ...
    \fill[red] (F1) circle (2pt) node[below right] {$F_1$};
    \fill[red] (F2) circle (2pt) node[below left] {$F_2$};
...
\begin{tikzpicture}[scale=1.3]
    ...
    % Vértices menores
    \fill[green!60!black] (0,2.5) circle (2pt) node[above left] {$B_1(0,b)$};
    \fill[green!60!black] (0,-2.5) circle (2pt) node[below right] {$B_2(0,-b)$};

    % Ejes
    \draw[{Latex}-{Latex}, thick] (-4.5,0) -- (4.5,0) node[above left=2mm] {Eje mayor};
    \draw[{Latex}-{Latex}, thick] (0,-3) -- (0,3) node[above] {Eje menor};

    % Medidas
    \draw[|{Latex}-{Latex}|, accentcolor] (-4,-3.5) -- (4,-3.5) node[midway, below] {$2a$};
    \draw[|{Latex}-{Latex}|, accentcolor] (4.8,0) -- (4.8,2.5) node[midway, right] {$b$};
\end{tikzpicture}
...
La excentricidad mide qué tan alargada es la elipse:
\[
...
\textbf{Paso 7:} Verificación usando un punto en la elipse.
...
\textbf{Paso 8:} Verificación de la definición de elipse.
...
\node[above left] at (axis cs:0,0) {$C(0,0)$};

% Focos
\addplot[only marks, mark=*, mark size=3pt, red] coordinates {(-5.476,0) (5.476,0)};
\node[below right=1mm] at (axis cs:-5.476,0) {$F_1$};
\node[below left=1mm] at (axis cs:5.476,0) {$F_2$ (Sol)};

% Vértices mayores
\addplot[only marks, mark=*, mark size=2pt, blue] coordinates {(-150,0) (150,0)};
\node[above right] at (axis cs:-150,0) {$V_1(-150,0)$};
\node[below left=2mm] at (axis cs:150,0) {$V_2(150,0)$};
...
Se está diseñando un estadio elíptico con centro en el punto $(20, 30)$ metros. El eje mayor mide 80 metros y es horizontal, mientras que el eje menor mide 60 metros. Encuentra la ecuación de la elipse, las coordenadas de los focos y calcula la excentricidad del diseño.
...
Para una elipse con centro en $(h, k)$ y eje mayor horizontal:
\[
...
Esta excentricidad indica una elipse moderadamente alargada.
...
Verificamos que $V_2(60, 30)$ está en la elipse:
\[
...
\node[below left] at (axis cs:-6.46,30) {$F_1$};
...
\node[below right] at (axis cs:20,0) {$B_1$};
...
Una trayectoria elíptica está descrita por la ecuación $9x^2 + 25y^2 - 18x + 100y - 116 = 0$. Encuentra el centro, los vértices, los focos y la excentricidad de la elipse. Grafica la elipse identificando todos sus elementos.
...
\textbf{Paso 7:} Identificar los parámetros de la elipse.
...
Excentricidad alta ($e = 0.8$) indica una elipse bastante alargada.
...
\textbf{Paso 13:} Graficar la elipse con todos sus elementos.
...
\node[above right] at (axis cs:-3,-2) {$F_1(-3,-2)$};
\node[above left] at (axis cs:5,-2) {$F_2(5,-2)$};
...
\node[below right] at (axis cs:1,-5) {$B_1(1,-5)$};
\node[above right] at (axis cs:1,1) {$B_2(1,1)$};
...
\begin{ejemplo}[title={Elipse con Eje Mayor Vertical - Diseño Arquitectónico}]
Un arco elíptico en un edificio tiene su eje mayor vertical. La altura máxima del arco es de 12 metros y el ancho en su base es de 8 metros. Si se coloca el centro del arco en el origen, encuentra la ecuación de la elipse, los focos y determina la altura del arco a 2 metros del centro.
...
Para una elipse con centro en el origen y eje mayor vertical:
\[
...
\node[above right] at (axis cs:0,-6) {$V_1(0,-6)$};
\node[below right] at (axis cs:0,6) {$V_2(0,6)$};

% Vértices menores
\addplot[only marks, mark=*, mark size=2pt, green!60!black] coordinates {(-4,0) (4,0)};
\node[below left] at (axis cs:-4,0) {$B_1(-4,0)$};
\node[below right] at (axis cs:4,0) {$B_2(4,0)$};
...
En el diseño de una sala de conferencias elíptica, se desea que el sonido producido en un foco se refleje y llegue perfectamente al otro foco. La sala tiene 30 metros de largo y 20 metros de ancho. Si se coloca el centro de la elipse en el origen con el eje mayor horizontal, determina dónde se deben ubicar los dos focos (puntos óptimos para los oradores).
...
\textbf{Paso 3:} Escribir la ecuación de la elipse.
...
Por la propiedad de la elipse, cualquier onda sonora que salga de $F_1$ se refleja en las paredes elípticas y converge exactamente en $F_2$. Esto permite que:
\begin{itemize}[leftmargin=*]
...
Para cualquier punto $P$ en la elipse:
\[
...
\node[above right] at (axis cs:-11.18,0) {$F_1$ (Orador 1)};
\node[above=3mm] at (axis cs:11.18,0) {$F_2$ (Orador 2)};
...
La propiedad acústica de la elipse garantiza que:
\begin{enumerate}[leftmargin=*]
...
\begin{ejemplo}[title={Problema Inverso - Construcción de Elipse Dados Condiciones}]
Se requiere diseñar una elipse que cumpla las siguientes condiciones: tiene centro en $(3, -2)$, uno de sus focos está en $(7, -2)$ y uno de sus vértices mayores está en $(10, -2)$. Encuentra la ecuación completa de la elipse y todos sus elementos.
...
Para una elipse con eje mayor horizontal y centro en $(h, k) = (3, -2)$:
\[
...
Verificamos que $V_2(10, -2)$ esté en la elipse:
\[
...
\textbf{Paso 10:} Graficar la elipse.
...
Los siguientes ejercicios requieren que trabajes "al revés": dadas ciertas condiciones o resultados, debes encontrar la elipse original. Estos problemas desarrollan tu razonamiento matemático y tu capacidad de análisis.

\begin{ejemplo}[title={Ejercicio Inverso 1: Dados los Focos y un Punto}]
Se sabe que una elipse tiene focos en $F_1(-3, 2)$ y $F_2(5, 2)$, y pasa por el punto $P(1, 6)$. Encuentra la ecuación de la elipse.
...
\textbf{Paso 3:} Usar la definición de elipse para encontrar $2a$.
...
Una elipse tiene excentricidad $e = \frac{3}{5}$, centro en el origen y sus vértices mayores en $(\pm 10, 0)$. Encuentra la ecuación de la elipse y las coordenadas de los focos.
...
\textbf{Paso 4:} Escribir la ecuación de la elipse.
...
\begin{ejemplo}[title={Ejercicio Inverso 3: Elipse que Pasa por Tres Puntos}]
Encuentra la ecuación de la elipse con centro en el origen, eje mayor horizontal, que pasa por los puntos $A(4, 0)$, $B(0, 3)$ y $C(2\sqrt{2}, \sqrt{6})$.
...
\textbf{¡Problema!} El tercer punto no está en la elipse propuesta. Esto significa que los tres puntos NO definen una elipse con esas características específicas.

\textbf{Paso 5:} Replanteamiento del problema.

Si los tres puntos DEBEN estar en la elipse, entonces $A(4,0)$ y $B(0,3)$ NO son necesariamente vértices. Debemos usar la forma general:
\[
\frac{x^2}{a^2} + \frac{y^2}{b^2} = 1
\]
...
\textbf{Conclusión:} Los tres puntos dados NO pueden estar simultáneamente en una elipse con centro en el origen y eje mayor horizontal que pase exactamente por $A$ y $B$ como vértices.

\textbf{Respuesta:}
\[
\boxed{\text{No existe tal elipse (puntos incompatibles con las condiciones dadas)}}
\]
...
Se sabe que para todos los puntos de una elipse, la suma de distancias a dos focos fijos es 26. Si los focos están en $F_1(0, -5)$ y $F_2(0, 5)$, encuentra la ecuación de la elipse.

\vspace{0.3cm}
\textbf{Solución:}

\textbf{Paso 1:} Usar la definición de elipse.
...
\textbf{Paso 5:} Escribir la ecuación de la elipse.
